% ===========================================================================
\chapter{Terminology}\jchap{専門用語}
% [o] LABEL
\label{chap:Terminology}
\label{sec:Terminology}
% [o] INDEX DESTINATIOn (DEF)
\ifor{terminology}{専門用語}{せんもんようご}{Fachbegriffe}
% [o] INDEX TARGET

The following sections (ordered Latin alphabetically) can be used by itself to
understand some key concepts of Japanese language by explaining keywords
{専門用語} {【せんもんようご】}.

% A
% B
% C
% D
\input{../share/sec/Dakuten}    % label sec:Dakuten
\input{../share/sec/Diphthong}  % label sec:Diphthong
% E
% F
\input{../share/sec/Furigana}   % label sec:Furigana
% G
\input{../share/sec/Gojuonzu}   % label sec:Gojuonzu
% H
\input{../share/sec/Handakuten} % label sec:Handakuten
\input{../share/sec/Hentaigana} % label sec:Hentaigana
\input{../share/sec/Hepburn}    % label sec:Hepburn
\input{../share/sec/Hiragana}   % label sec:Hiragana
\input{../share/sec/Homophone}  % label sec:Homophone
% I
\input{../share/sec/Iroha}      % label sec:Iroha
\input{../share/sec/Iteration}  % label sec:Iteration
% J
% K
\input{../share/sec/Kana}       % label sec:Kana
\input{../share/sec/Kanji}      % label sec:Kanji
\input{../share/sec/Katakana}   % label sec:Katakana
\input{../share/sec/Kunrei}     % label sec:Kunrei
% L
% M
\input{../share/sec/Manga}      % label sec:Manga
\input{../share/sec/Manyogana}  % label sec:Manyogana
\input{../share/sec/Mora}       % label sec:Mora
% N
% O
\input{../share/sec/Okurigana}  % label sec:Okurigana
% P
\input{../share/sec/PhoneticCharacter} % label sec:PoneticCharacter
% Q
% R
\input{../share/sec/Radical}    % label sec:Radical
\input{../share/sec/Romaji}     % label sec:Romaji
% S
\input{../share/sec/SpaceCharacter} % sec:SpaceCharacter
\input{../share/sec/Syllable}   % label sec:Syllable
% T
% U
% V
% W
% X
% Y
% Z

